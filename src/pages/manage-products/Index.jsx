...
import { Checkbox, InputGroup, Avatar, InputRightElement, Input, VStack, Badge, Box } from "@chakra-ui/react";
...
import { productsTable } from "../../redux/redux-slice/tableData.slice";
const Index = () => {
    ...
    const data = useSelector((state) => state.tableDataSlice.product)
    console.log(data);
    const getAllProduct = async () => {
        ...
        try {
            const getAllProductResponse = await productApi.getAllProduct()
            if (getAllProductResponse.data.code === 200) {
                dispatch(productsTable(getAllProductResponse.data.data));
                setLoading(false);
            } else {
                toast.error(getAllProductResponse.data.message);
            }
        } catch (error) {
    ...
    const DeleteHandler = async (id) => {
        try {
            const getAllProductResponse = await productApi.deleteProductById({
                productId: id
            })
            if (getAllProductResponse.data.code === 200) {
                toast.success(getAllProductResponse.data.message);
                getAllProduct()
            } else {
                toast.error(getAllProductResponse.data.message);
            }
                ...
                <div className="w-full  ">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <button type="button"
                            ...
                            onClick={() => Navigate('/manage-products/product/register')}>
                                        ...
                                        {rowsToShow?.map((data, index) => {
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className={` px-3 font-normal text-base `} >
                                                        <Avatar w={10} h={10} src={data?.coverImage} />
                                                    </td>
                                                    ...
                                                    <td
                                                        ...
                                                        <DropdownEditMenu align="right" className="relative inline-flex">
                                                            <li>
                                                                <Link to={`/manage-products/product/register/?id=${data?.productId}`}>
                                                                ...
                                                                <div className="font-medium text-sm  text-red-500 cursor-pointer flex py-1 px-3" to="#0"
                                                                    onClick={() => DeleteHandler(data?.productId)}>
                                                                    Delete
                                                                </div>
                                                            </li>
                                                        </DropdownEditMenu>
