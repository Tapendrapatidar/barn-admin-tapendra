...
import SubCategorysApi from "../../apis/products.api";
import {  productsSubCatagoryTable } from "../../redux/redux-slice/tableData.slice";
const Index = () => {
    ...
    const SubcategoryApi = new SubCategorysApi()
    ...
    const data = useSelector((state) => state.tableDataSlice.productSubCategory)
    const getAllSubCategory = async () => {
        if (data.length === 0) {
            setLoading(true)
        }
        try {
            const getAllSubCategoryResponse = await SubcategoryApi.getAllSubCategory()
            if (getAllSubCategoryResponse.data.code === 200) {
                dispatch(productsSubCatagoryTable(getAllSubCategoryResponse.data.data));
                setLoading(false);
            } else {
    ...
    const DeleteHandler = async (id) => {
        try {
            const getAllSubCategoryResponse = await SubcategoryApi.deleteSubCategoryById({
                subCategoryId: id
            })
            if (getAllSubCategoryResponse.data.code === 200) {
                toast.success(getAllSubCategoryResponse.data.message);
                getAllSubCategory()
            } else {
                toast.error(getAllSubCategoryResponse.data.message);
            }
                ...
                <div className="w-full  ">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <button type="button"
                            ...
                            onClick={() => Navigate('/manage-products/sub-category/register')}>
                                        ...
                                        {rowsToShow?.map((data, index) => {
                                            return (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <Avatar w={10} h={10} src={data?.image} />
                                                    </td>
                                                    <td
                                                    ...
                                                    <td
                                                        ...
                                                        <DropdownEditMenu align="right" className="relative inline-flex">
                                                            <li>
                                                                <Link to={`/manage-products/sub-category/register/?id=${data?.subCategoryId}`}>
                                                                ...
                                                                <div className="font-medium text-sm  text-red-500 cursor-pointer flex py-1 px-3" to="#0"
                                                                    onClick={() => DeleteHandler(data?.subCategoryId)}>
                                                                    Delete
                                                                </div>
                                                            </li>
                                                        </DropdownEditMenu>
