...
import { adminTable } from "../../redux/redux-slice/tableData.slice";
...
const Index = () => {
    ...
    const data = useSelector((state) => state.tableDataSlice.admin)
    const getAllAdmin = async () => {
        if (data.length === 0) {
            setLoading(true)
        }
        try {
            const getAllAdminResponse = await adminApi.getAllAdmins()
            if (getAllAdminResponse.data.code === 200) {
                dispatch(adminTable(getAllAdminResponse.data.data));
                setLoading(false);
            } else {
                toast.error(getAllAdminResponse.data.message);
            }
        } catch (error) {
    ...
    const DeleteHandler = async (id) => {
        try {
            const getAllAdminResponse = await adminApi.deleteAdminById({
                adminId: id
            })
            if (getAllAdminResponse.data.code === 200) {
                toast.success(getAllAdminResponse.data.message);
                getAllAdmin()
            } else {
                toast.error(getAllAdminResponse.data.message);
            }
                        ...
                        <table className="table-auto overflow-scroll md:overflow-auto w-full text-left font-inter  ">
                            <thead className="rounded-lg text-base text-white font-semibold w-full">
                                <tr className="bg-white border-b">
                                    ...
                                    <th className="py-2 px-3  justify-center gap-1 text-[#343a40] dark:text-gray-400 sm:text-base font-bold whitespace-nowrap">
                                        Email
                                    </th>
                                    <th className="py-2 px-3 text-[#343a40] dark:text-gray-400 sm:text-base font-bold whitespace-nowrap">
                                        Phone Number
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <DropdownEditMenu align="right" className="relative inline-flex">
                                                                <li>
                                                                    <Link to={`/admin/register/?id=${data?.staffId}`}>
